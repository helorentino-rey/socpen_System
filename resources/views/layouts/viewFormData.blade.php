...
<body>
    <div class="header">
        <div class="logos">
            ...
            <img src="{{ public_path('img/social-pension-logo.png') }}" alt="Social Pension Logo" class="social-pension-logo">
            <img src="{{ public_path('img/BagongPilipinas.png') }}" alt="Bagong Pilipinas Logo" class="bagong-pilipinas-logo">
        </div>
    ...
    <div class="container mt-5">
        ...
        <div class="container mt-5">
            ...
            <table class="table">
                ...
                <tr>
                    <td colspan="1">Last Name (Apelyido): <span>{{ $beneficiary->BeneficiaryInfo->last_name }}</span></td>
                    <td colspan="2">First Name (Unang Pangalan): <span>{{ $beneficiary->BeneficiaryInfo->first_name }}</span></td>
                    <td colspan="1">Middle Name (Gitnang Pangalan): <span>{{ $beneficiary->BeneficiaryInfo->middle_name }}</span></td>
                    <td colspan="1">Ext. (Jr., II): <span>{{ $beneficiary->BeneficiaryInfo->name_extension }}</span></td>
                </tr>
                <tr>
                    <th colspan="5">MOTHER'S MAIDEN NAME (Pangalan ng Ina sa Pagkadalaga):</th>
                </tr>
                <tr>
                    <td colspan="2">Last Name (Apelyido): <span>{{ $beneficiary->MothersMaidenName->mother_last_name }}</span></td>
                    <td colspan="2">First Name (Unang Pangalan): <span>{{ $beneficiary->MothersMaidenName->mother_first_name }}</span></td>
                    <td colspan="1">Middle Name (Gitnang Pangalan): <span>{{ $beneficiary->MothersMaidenName->mother_middle_name }}</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="1"><span>{{ \Carbon\Carbon::parse($beneficiary->MothersMaidenName->date_of_birth)->format('F j, Y') }}</p></span></td>
                    <td colspan="2">City/Municipality (Lungsod)<span>{{ $beneficiary->MothersMaidenName->place_of_birth_city }}</span></td>
                    <td colspan="2">Province (Lalawigan)<span>{{ $beneficiary->MothersMaidenName->place_of_birth_province }}</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="1">Affiliation Type: <span>{{ $beneficiary->affiliation->affiliation_type ?? 'N/A' }}</span></td>
                    <td colspan="2">Household ID: <span>{{ $beneficiary->affiliation->hh_id ?? 'N/A' }}</span></td>
                    <td colspan="2">Indigenous Specify: <span>{{ $beneficiary->affiliation->indigenous_specify ?? 'N/A' }}</span></td>
                </tr>
            </table>
            ...
            <table class="table">
                ...
                <tr>
                    <td colspan="1">Last Name (Apelyido): <span>{{ $beneficiary->spouse->spouse_last_name ?? 'N/A' }}</span></td>
                    <td colspan="2">First Name (Unang Pangalan): <span>{{ $beneficiary->spouse->spouse_first_name ?? 'N/A' }}</span></td>
                    <td colspan="1">Middle Name (Gitnang Pangalan): <span>{{ $beneficiary->spouse->spouse_middle_name ?? 'N/A' }}</span></td>
                    <td colspan="1">Ext. (Jr., II): <span>{{ $beneficiary->spouse->spouse_name_extension ?? 'N/A' }}</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td colspan="3">NAME(Pangalan): <span>{{ $representative->representative_name ?? 'N/A' }}</span></td>
                    <td colspan="1">CIVIL STATUS(Katayuang Sibil): <span>{{ $representative->representative_civil_status ?? 'N/A' }}</span></td>
                    <td colspan="1">CONTACT NUMBER(Numero ng Telepono): <span>{{ $representative->representative_contact_number ?? 'N/A' }}</span></td>
                    @endforeach
                </tr>
                <tr>
                ...
                <tr>
                    <td>Last Name (Apelyido): <span>{{ $beneficiary->caregiver->caregiver_last_name ?? 'N/A' }}</span></td>
                    <td>First Name (Unang Pangalan): <span> {{ $beneficiary->caregiver->caregiver_first_name ?? 'N/A' }}</span></td>
                    <td>Middle Name (Gitnang Pangalan): <span>{{ $beneficiary->caregiver->caregiver_middle_name ?? 'N/A' }}</span></td>
                    <td>Ext. (Jr., II): <span>{{ $beneficiary->caregiver->caregiver_name_extension ?? 'N/A' }}</span></td>
                </tr>
                <tr>
                ...
                <tr>
                    <td colspan="3">Housing Situation(Sitwasyon ng Pamamahay): <span>{{ $beneficiary->housingLivingStatus->house_status ?? 'N/A' }}</span></td>
                    <td colspan="2">Living Arrangement (kaayusan sa pamumuhay): <span>{{ $beneficiary->housingLivingStatus->living_status ?? 'N/A' }}</span></td>
                </tr>
            </table>
            ...
            <table class="table">
                ...
                <tr>
                    <td colspan="1">Yes(Oo)/No(Wala): <span>{{ $beneficiary->economicInformation->receiving_pension ?? 'N/A' }}</span></td>
                    <td colspan="1">How much (Magkano): <span>{{ $beneficiary->economicInformation->pension_amount ?? 'N/A' }}</span></td>
                    <td colspan="3">Source (Mula saan): <span>{{ $beneficiary->economicInformation->pension_source ?? 'N/A' }}</span></td>
                </tr>
                <tr>
                    <th colspan="5">PERMANENT INCOME (Permanenteng Kita):</th>
                </tr>
                <tr>
                    <td colspan="1">Yes(Oo)/No(Wala): <span>{{ $beneficiary->economicInformation->permanent_income ?? 'N/A' }}</span></td>
                    <td colspan="1">How much (Magkano): <span>{{ $beneficiary->economicInformation->income_amount ?? 'N/A' }}</span></td>
                    <td colspan="3">Source (Mula saan): <span>{{ $beneficiary->economicInformation->income_source ?? 'N/A' }}</span></td>
                </tr>
                <tr>
                    <th colspan="5">REGULAR SUPPORT (Regular na Suporta):</th>
                </tr>
                <tr>
                    <td colspan="1">Yes(Oo)/No(Wala): <span>{{ $beneficiary->economicInformation->regular_support ?? 'N/A' }}</span></td>
                    <td colspan="1">How much (Magkano): <span>{{ $beneficiary->economicInformation->support_amount ?? 'N/A' }}</span></td>
                    <td colspan="3">Source (Mula saan): <span>{{ $beneficiary->economicInformation->support_source ?? 'N/A' }}</span></td>
                </tr>
            </table>
            ...
            <table class="table">
                ...
                <tr>
                    <td colspan="2">Yes(Oo)/No(Wala): <span>{{ $beneficiary->healthInformation->existing_illness ?? 'N/A' }}</span></td>
                    <td colspan="3">Specify (Itala): <span>{{ $beneficiary->healthInformation->illness_specify ?? 'N/A' }}</span></td>
                </tr>
                <tr>
                    <th colspan="5">WITH DISABILITY (May Kapansanan):</th>
                </tr>
                <tr>
                    <td colspan="2">Yes(Oo)/No(Wala): <span>{{ $beneficiary->healthInformation->with_disability ?? 'N/A' }}</span></td>
                    <td colspan="3">Specify (Itala): <span>{{ $beneficiary->healthInformation->disability_specify ?? 'N/A' }}</span></td>
                </tr>
                <tr>
                    <th colspan="5">FRAILTY QUESTIONS:</th>
                </tr>
                <tr>
                    <td>1. Do you experience difficulty in doing your ADLs? (Yes/No) <span>{{ $beneficiary->healthInformation->difficult_adl ?? 'N/A' }}</span></td>
                    <td>2. Are you completely dependent on someone in doing your IADLs? (Yes/No)<span>{{ $beneficiary->healthInformation->dependent_iadl ?? 'N/A' }}</span></td>
                    <td>3. Are you experiencing weight loss, weakness, exhaustion? (Yes/No)<span>{{ $beneficiary->healthInformation->experience_loss ?? 'N/A' }}</span></td>
                </tr>
            </table>
